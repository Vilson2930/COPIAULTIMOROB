...
def status_color(value):
    ...
    if any(x in v for x in ["REPROVADO", "CRITICO", "CRÍTICO", "ALTO", "FAIL", "BLOQUEAR", "CRITICA", "CRÍTICA"]):
        return "#ef4444"
    if any(x in v for x in ["RESSALVA", "MEDIO", "MÉDIO", "MODERADO", "ATENCAO", "ATENÇÃO", "FRAGIL", "FRÁGIL", "MEDIA", "MÉDIA", "DESALINHADO"]):
        return "#facc15"
    if any(x in v for x in ["APROVADO", "VALIDADO", "BAIXO", "NORMAL", "OK", "INSTITUCIONAL", "ROBUSTO", "ACEITAVEL", "COERENTE", "ALINHADO"]):
        return "#22c55e"
    return "#38bdf8"
    ...
    if not allocation_rows.empty:
        for _, row in allocation_rows.head(10).iterrows():
            ...
            <tr>
                ...
                <td style="padding:10px;border-bottom:1px solid #1f2937;color:{status_color(row.get("prioridade_modelo", ""))};font-weight:bold;">{fmt(row.get("desvio_pct"))}%</td>
                <td style="padding:10px;border-bottom:1px solid #1f2937;color:{status_color(row.get("prioridade_modelo", ""))};font-weight:bold;">{row.get("acao_modelo", "N/D")}</td>
                <td style="padding:10px;border-bottom:1px solid #1f2937;color:#d1d5db;">{row.get("prioridade_modelo", "N/D")}</td>
            </tr>
            """
    else:
        allocation_table_rows = """
        <tr>
            <td colspan="6" style="padding:10px;color:#d1d5db;">Allocation Advisor não disponível.</td>
        </tr>
        """
...
<div style="max-width:960px;margin:28px auto;background:#111827;border:1px solid #1f2937;border-radius:22px;padding:34px;">
    ...
    <div style="background:#020617;border:1px solid #1f2937;border-radius:14px;padding:20px;color:#d1d5db;line-height:1.7;">
        <ol style="margin:0;padding-left:22px;">
            <li>Restaurar runway operacional para no mínimo 12 meses quando aplicável.</li>
